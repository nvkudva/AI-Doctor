...
function demoComplete(args: { system: string; messages: ChatMessage[] }): Promise<string> {
  const isDocReview = /reviewing your recommendation/i.test(args.system || '');
  const raw = isDocReview ? demoDoctor(args.system, args.messages) : demoPatient(args.messages);
...
function demoDoctor(system: string, messages: ChatMessage[]): any {
  ...
  if (rec && /\b(remove|drop|take out|delete)\b/i.test(text)) {
    ...
    const idx = items.findIndex((it: any) => it.name && text.toLowerCase().includes(it.name.split(' ')[0].toLowerCase()));
    const cut = idx >= 0 ? idx : items.length - 1;
    if (items.length > 1) {
      const removed = items[cut];
      const rest = items.filter((_: any, i: number) => i !== cut);
